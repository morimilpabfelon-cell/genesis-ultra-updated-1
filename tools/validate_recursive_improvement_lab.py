...
def validate_authority(lab,guided):
 ...
 opened=lab['campaign']['opened_at'];campaign_sig=sign_fixture_envelope(guided['keys']['guardian'],'guardian',guided['guardian_id'],lab['campaign']['campaign_digest'],'genesis.improvement.campaign.signature.v0.2',opened);verify_envelope(campaign_sig,guided['keys']['guardian'],digest=lab['campaign']['campaign_digest'],domain='genesis.improvement.campaign.signature.v0.2',signer_type='guardian',signer_id=guided['guardian_id'],created_at=opened,prefix='campaign')
 request={'campaign_digest':lab['campaign']['campaign_digest'],'grant_ref':lab['campaign']['guardian_grant_ref'],'instance_id':lab['campaign']['instance_id'],**binding,'authorized_at':opened}
 receipt=authorize_campaign_opening(request,authority)
 if receipt['decision_status']!='allowed' or receipt['grant_ref']!=lab['campaign']['guardian_grant_ref']: raise ValueError('campaign_authorization_failed:'+receipt['decision_reason'])
 use={'schema_version':'genesis.autonomy.capability.use.v0.2','hash_profile':'genesis.hash.fields.v0.1','use_id':'use_01HRILAB_COMPILE001','grant_ref':lab['campaign']['guardian_grant_ref'],'instance_id':lab['campaign']['instance_id'],'body_id':body,'capability':binding['capability'],'target_ref':binding['target_ref'],'action_class':binding['action_class'],'data_class':binding['data_class'],'requested_actions':1,'requested_duration_seconds':30,'requested_bytes':1000000,'sandboxed':binding['sandboxed'],'human_confirmation_ref':binding['human_confirmation_ref'],'observer_ref':binding['observer_ref'],'reversible_plan_ref':binding['reversible_plan_ref'],'requested_at':plus(opened,1),'use_digest':'','signature':None};use['use_digest']=compute_authorized_use_digest(use);use['signature']=sign_fixture_envelope(guided['keys']['body'],'body',body,use['use_digest'],'genesis.autonomy.capability.use.signature.v0.2',use['requested_at']);decision=evaluate_authorized_use(use,authority)
 ...
 bad_sig=deepcopy(campaign_sig);bad_sig['signature_value']='0'*128;expect_failure('campaign_signature',lambda:verify_envelope(bad_sig,guided['keys']['guardian'],digest=lab['campaign']['campaign_digest'],domain='genesis.improvement.campaign.signature.v0.2',signer_type='guardian',signer_id=guided['guardian_id'],created_at=opened,prefix='campaign'));negatives+=1;bad_use=deepcopy(use);bad_use['grant_ref']='grant_other';expect_failure('signed_grant_ref',lambda:evaluate_authorized_use(bad_use,authority));negatives+=1
