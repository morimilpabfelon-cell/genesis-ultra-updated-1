...
`sense_observation_vectors.json`, `sense_adapter_vectors.json`,
`continuity_vectors.json` y `crypto_vectors.json`.

Estado actual de implementaciones de referencia:

| Lenguaje | Ubicación | Cubre | Estado |
|---|---|---|---|
| Python | `tools/*.py` | workspace, identidad inmutable, sentidos/compuerta, adaptadores neutrales, continuidad, contrato host, cripto-digests, simulaciones A→B y negativas | activa |
| JavaScript (Node) | `tools/*.mjs` | workspace, identidad inmutable, sentidos/compuerta, adaptadores neutrales, contrato host, todos los vectores compartidos, artefactos A→B/recovery y journal | activa |
